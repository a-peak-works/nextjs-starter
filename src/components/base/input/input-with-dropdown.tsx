    ...
    return (
        <div className="relative inline-flex h-full items-center">
            <select
                ...
                className={cx(
                    "flex h-full w-min appearance-none items-center gap-1 bg-inherit px-3 py-2 text-md text-tertiary outline-hidden ring-inset focus:ring-2 focus:ring-brand disabled:cursor-not-allowed",
                    props.isInvalid && "focus:ring-2 focus:ring-error",
        ...
        >
            {({ isDisabled, isInvalid, isRequired }) => (
                <>
                    ...
                    <div
                        className={cx(
                            "relative flex h-max w-full flex-row justify-start rounded-lg bg-primary shadow-xs ring-1 ring-primary transition-all duration-100 ease-linear ring-inset",

                            // Only apply focus ring when child input is focused
                            "has-[input:focus]:ring-2 has-[input:focus]:ring-brand",

                            isDisabled && "cursor-not-allowed bg-disabled_subtle ring-disabled",
                            isInvalid && "ring-error_subtle has-[input:focus]:ring-error",
                        )}
                    >
